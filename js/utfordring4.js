...
// Utfordring 3
var SettMegTilSantHvisPizzaErFerdig = false;
var BeskjedKanSendes = 0;
...
function PizzaDeigKode() {
	...
	try {
		SettMegTilSantHvisPizzaErFerdig = false;
		BeskjedKanSendes = 0;
		kodetest();
	} catch(err) {
		DetteSmakte(3);
...
function DetteSmakte(runde) {
	...
	if (runde == 1) {
		...
		if (!SettMegTilSantHvisPizzaErFerdig || BeskjedKanSendes != 2) {
		...
		} else if (SettMegTilSantHvisPizzaErFerdig && BeskjedKanSendes == 2) {
		...
		smak = 'Se om du klarer å få koden til å velge slik at familien får beskjed om middag!';
...
function SendBeskjedTilAlle_KomOgSpisMiddag() {}
